...
public class RingBuffer {
    ...
    public RingBuffer(int bufferSize, int paddingFactor) {
        ...
        if(bufferSize <= 0)
        { throw new IdGenerateException("RingBuffer size must be positive");

        }
        if(Integer.bitCount(bufferSize) != 1) {
            throw new IdGenerateException("RingBuffer size must be a power of 2");
        }
        if((paddingFactor > 0 && paddingFactor < 100)==false)
        {
            throw new IdGenerateException("RingBuffer size must be positive");
        }
    ...
    public long take() {
        ...
        if(nextCursor < currentCursor) {
            throw new IdGenerateException("Curosr can't move back");
        }
        ...
        if((flags[nextCursorIndex].get() == CAN_TAKE_FLAG)==false)
        {
            throw new IdGenerateException("Curosr not in can take status");
        }
